...
@patch("src.core.converters.office.get_pandoc_path")
@patch("subprocess.run")
def test_convert_docx_fallback_pandoc(
    mock_run, mock_pandoc, mock_soffice, converter, tmp_path
):
...
@patch("src.core.converters.office.get_pandoc_path")
@patch("subprocess.run")
def test_convert_soffice_failure_retry(
    mock_run, mock_pandoc, mock_soffice, converter, tmp_path
):
    mock_soffice.return_value = "/usr/bin/soffice"
    mock_pandoc.return_value = "/usr/bin/pandoc"
...
@patch("src.core.converters.office.get_pandoc_path")
@patch("subprocess.run")
def test_convert_soffice_no_html_generated(
    mock_run, mock_pandoc, mock_soffice, converter, tmp_path
):
    mock_soffice.return_value = "/usr/bin/soffice"
    mock_pandoc.return_value = "/usr/bin/pandoc"
...
@patch("src.core.converters.office.get_pandoc_path")
@patch("subprocess.run")
def test_convert_pandoc_missing_fallback(
    mock_run, mock_pandoc, mock_soffice, converter, tmp_path
):
...
@patch("src.core.converters.office.get_pandoc_path")
@patch("subprocess.run")
def test_pandoc_exit_21_fallback(
    mock_run, mock_pandoc, mock_soffice, converter, tmp_path
):
    mock_soffice.return_value = "/usr/bin/soffice"
    mock_pandoc.return_value = "/usr/bin/pandoc"
    ...
    def side_effect(cmd, **kwargs):
        if "soffice" in cmd[0]:
            ...
            output_file.write_text(
                "Some <span class='x'>text</span> and <div>div</div>", encoding="utf-8"
            )
        return MagicMock(returncode=0)
