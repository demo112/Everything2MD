...
def test_pdf_conversion_pandoc_success(ppt_converter, tmp_path):
    ...
    with patch('src.core.converters.ppt.get_pandoc_path', return_value='/usr/bin/pandoc'), \
...
def test_pdf_conversion_all_cmds_fail_pdfminer_success(ppt_converter, tmp_path):
    ...
    with patch('src.core.converters.ppt.get_pandoc_path', return_value='/usr/bin/pandoc'), \
         ...
         patch('pdfminer.high_level.extract_text', return_value="Extracted text", create=True):
...
def test_fallback_pdf_parsing_copy_on_failure(ppt_converter, tmp_path):
    ...
    with patch('pdfminer.high_level.extract_text', side_effect=ImportError, create=True), \
