...
def test_upload_selected_files_success(mock_exists, app):
    ...
    with patch("threading.Thread") as MockThread:
        ...
        app.ragflow_client.run_parsing.assert_called_once_with(
...
def test_upload_deduplication(mock_exists, app):
    ...
    # Setup
    app.kb_combo.get.return_value = "MyKB"
    app.converted_files.append(
        {
            "name": "existing_file.docx",
            "path": "/path/to/existing_file.docx",
            "status": "Ready",
            "id": "",
        }
    )
    ...
    app.rag_file_list.get_children.return_value = children
    app.rag_file_list.set.return_value = "☑"
    app.rag_file_list.item.return_value = {
        "values": ["☑", "existing_file.docx", "Ready", "Not Uploaded"]
    }
