...
class Everything2MDGUI:
    def __init__(self, root):
        ...
        # 图片识别变量
        self.img_rec_enabled = tk.BooleanVar(value=False)
        self.img_rec_api_base = tk.StringVar(value="https://api.openai.com/v1")
        self.img_rec_api_key = tk.StringVar()
        ...
        logging.getLogger().setLevel(
            getattr(logging, current_level.upper(), logging.INFO)
        )
    ...
    def handle_tk_exception(self, exc_type, exc_value, exc_traceback):
        ...
        logging.getLogger().critical(
            "Tkinter exception:", exc_info=(exc_type, exc_value, exc_traceback)
        )
    ...
    def create_widgets(self):
        ...
        # Tab 3: Parsing Enhancement
        self.parsing_frame = ttk.Frame(self.notebook, padding="10")
        self.notebook.add(self.parsing_frame, text="解析增强")
        self.init_parsing_tab(self.parsing_frame)
    ...
    def init_convert_tab(self, parent):
        ...
        self.browse_input_button = ttk.Button(
            btn_frame, text="选择文件", width=10, command=self.browse_input
        )
        self.browse_input_button.pack(side=tk.LEFT, padx=2)
        self.browse_input_dir_button = ttk.Button(
            btn_frame, text="选择目录", width=10, command=self.browse_input_dir
        )
        ...
        self.output_entry.grid(
            row=1, column=1, sticky=(tk.W, tk.E), pady=2, padx=(0, 5)
        )
        self.browse_output_button = ttk.Button(
            parent, text="浏览...", command=self.browse_output
        )
        ...
        # 日志级别
        ttk.Label(config_frame, text="日志级别:").grid(
        ...
        self.log_level_combo.grid(
            row=0, column=1, sticky=(tk.W, tk.E), pady=2, padx=(0, 5)
        )

        # 输出格式
        ttk.Label(config_frame, text="输出格式:").grid(
        ...
        # 批量处理
        ttk.Label(config_frame, text="批量处理:").grid(
            row=1, column=0, sticky=tk.W, pady=2
        )
        self.batch_checkbox = ttk.Checkbutton(
            config_frame, text="启用", variable=self.batch_processing
        )
        self.batch_checkbox.grid(row=1, column=1, sticky=tk.W, pady=2)

        # 并行任务数
        ttk.Label(config_frame, text="并行任务数:").grid(
            row=1, column=2, sticky=tk.W, pady=2
        )
        self.max_jobs_spinbox = ttk.Spinbox(
            config_frame, textvariable=self.max_parallel_jobs, from_=1, to=16, width=5
        )
        self.max_jobs_spinbox.grid(row=1, column=3, sticky=tk.W, pady=2)

        # 文件过滤器
        ttk.Label(config_frame, text="文件过滤器:").grid(
        ...
        filter_container.grid(
            row=2, column=1, columnspan=3, sticky=(tk.W, tk.E), pady=2
        )

        # Scan Button
        self.scan_button = ttk.Button(
            filter_container, text="扫描类型", command=self.scan_file_types
        )
        ...
        # LibreOffice 路径
        ttk.Label(config_frame, text="LibreOffice:").grid(
            row=3, column=0, sticky=tk.W, pady=2
        )
        self.soffice_entry = ttk.Entry(config_frame, textvariable=self.soffice_path)
        self.soffice_entry.grid(
            row=3, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=2, padx=(0, 5)
        )
        self.browse_soffice_button = ttk.Button(
            config_frame, text="...", width=3, command=self.browse_soffice
        )
        self.browse_soffice_button.grid(row=3, column=3, sticky=tk.W, pady=2)

        # Pandoc 路径
        ttk.Label(config_frame, text="Pandoc:").grid(
            row=4, column=0, sticky=tk.W, pady=2
        )
        self.pandoc_entry = ttk.Entry(config_frame, textvariable=self.pandoc_path)
        self.pandoc_entry.grid(
            row=4, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=2, padx=(0, 5)
        )
        self.browse_pandoc_button = ttk.Button(
            config_frame, text="...", width=3, command=self.browse_pandoc
        )
        ...
        self.cancel_button = ttk.Button(
            button_frame, text="取消", command=self.cancel_conversion, state=tk.DISABLED
        )
        ...
        self.file_status_tree = ttk.Treeview(
        ...
        list_scroll = ttk.Scrollbar(
            list_frame, orient=tk.VERTICAL, command=self.file_status_tree.yview
        )
        ...
        scrollbar = ttk.Scrollbar(
            log_frame, orient=tk.VERTICAL, command=self.status_text.yview
        )
    ...
    def set_controls_disabled(self, disabled):
        ...
        for w in [
            ...
            self.soffice_entry,
            self.browse_soffice_button,
            self.pandoc_entry,
            self.browse_pandoc_button,
        ]:
            try:
    ...
    def scan_file_types(self):
        ...
        def _scan():
            ...
            self.root.after(
                0,
                lambda: [
                    self.refresh_filter_checkboxes(list(extensions.keys()), extensions),
    ...
    def refresh_filter_checkboxes(self, extensions, counts=None):
        ...
        current_filters = [
            f.strip() for f in self.file_filters.get().split(",") if f.strip()
        ]
    ...
    def update_file_status(self, file_path, status, message):
        self.root.after(
            0, lambda: self._update_file_status_ui(file_path, status, message)
        )
    ...
    def init_rag_tab(self, parent):
        ...
        ttk.Entry(cfg_frame, textvariable=self.rag_api_base).grid(
            row=0, column=1, sticky="ew", padx=5
        )

        ttk.Label(cfg_frame, text="API Key:").grid(row=1, column=0, sticky="w")
        ttk.Entry(cfg_frame, textvariable=self.rag_api_key, show="*").grid(
            row=1, column=1, sticky="ew", padx=5
        )

        ttk.Button(cfg_frame, text="连接/刷新KB", command=self.refresh_kb_list).grid(
            row=1, column=2
        )
        ...
        self.kb_combo = ttk.Combobox(
            ctrl_frame, textvariable=self.selected_kb_id, state="readonly", width=30
        )
        self.kb_combo.pack(side="left", padx=5)

        ttk.Button(
            ctrl_frame, text="新建知识库...", command=self.show_new_kb_dialog
        ).pack(side="left", padx=5)
        ttk.Button(
            ctrl_frame, text="上传选中文件", command=self.upload_selected_files
        ).pack(side="right", padx=5)
        ttk.Button(
            ctrl_frame, text="全选/反选", command=self.toggle_all_selection
        ).pack(side="right", padx=5)
        ...
        self.rag_file_list = ttk.Treeview(
        ...
        scrollbar = ttk.Scrollbar(
            list_frame, orient=tk.VERTICAL, command=self.rag_file_list.yview
        )
    ...
    def init_parsing_tab(self, parent):
        ...
        # 1. Image Recognition Config
        img_frame = ttk.LabelFrame(
            parent, text="多模态大模型配置 (图片识别)", padding="10"
        )
        img_frame.grid(row=0, column=0, sticky="ew", pady=5)
        img_frame.columnconfigure(1, weight=1)

        # Enable Switch
        ttk.Checkbutton(
            img_frame, text="启用图片识别功能", variable=self.img_rec_enabled
        ).grid(row=0, column=0, columnspan=2, sticky="w", pady=5)

        # API Base
        ttk.Label(img_frame, text="API Base URL:").grid(
            row=1, column=0, sticky="w", pady=2
        )
        ttk.Entry(img_frame, textvariable=self.img_rec_api_base).grid(
            row=1, column=1, sticky="ew", pady=2
        )

        # API Key
        ttk.Label(img_frame, text="API Key:").grid(row=2, column=0, sticky="w", pady=2)
        ttk.Entry(img_frame, textvariable=self.img_rec_api_key, show="*").grid(
            row=2, column=1, sticky="ew", pady=2
        )

        # Model
        ttk.Label(img_frame, text="模型名称:").grid(row=3, column=0, sticky="w", pady=2)
        ttk.Entry(img_frame, textvariable=self.img_rec_model).grid(
            row=3, column=1, sticky="ew", pady=2
        )

        # Concurrency
        ttk.Label(img_frame, text="最大并发数:").grid(
            row=4, column=0, sticky="w", pady=2
        )
        spin = ttk.Spinbox(
            img_frame, textvariable=self.img_rec_concurrency, from_=1, to=10, width=5
        )
    ...
    def toggle_all_selection(self):
        ...
        any_unchecked = any(
            "☐" in self.rag_file_list.set(item, "select") for item in children
        )
    ...
    def refresh_kb_list(self):
        ...
        def _do_refresh():
            try:
                ...
                log_info("知识库列表刷新成功")
                self.root.after(
                    0, lambda: self.status_bar.config(text="知识库列表刷新成功")
                )
            except Exception as e:
                log_info(f"刷新失败: {e}")
                self.root.after(
    ...
    def show_new_kb_dialog(self):
        ...
        ttk.Combobox(top, textvariable=tpl_var, values=kbs).grid(
        ...
        ttk.Button(top, text="创建", command=_create).grid(
    ...
    def upload_selected_files(self):
        ...
        def _bg_upload():
            self.root.after(
                0, lambda: self.status_bar.config(text="正在获取知识库文档列表...")
            )
            ...
            self.root.after(
                ...
                lambda: self.status_bar.config(
                    text=f"正在上传 {len(files_to_upload)} 个文件..."
                ),
            )
            success_count = 0
            fail_count = 0
            ...
            for iid, f_obj in files_to_upload:
                ...
                try:
                    ...
                    self.root.after(
                        0,
                        lambda i=iid: self.rag_file_list.set(
                            i, column="upload_status", value="上传中..."
                        ),
                    )
                    ...
                    if doc_ids:
                        self.root.after(
                            0,
                            lambda i=iid: self.rag_file_list.set(
                                i, column="upload_status", value="触发解析..."
                            ),
                        )
                        ...
                        self.root.after(
                            0,
                            lambda i=iid: self.rag_file_list.set(
                                i, column="upload_status", value="已启动解析"
                            ),
                        )
                    else:
                        ...
                        self.root.after(
                            0,
                            lambda i=iid: self.rag_file_list.set(
                                i, column="upload_status", value="上传完成(未解析)"
                            ),
                        )
                    ...
                    self.root.after(
                        0,
                        lambda i=iid: self.rag_file_list.set(
                            i, column="upload_status", value=short_msg
                        ),
                    )
            ...
            msg = (
                f"上传完成: 成功 {success_count}, 失败 {fail_count}, 跳过 {skip_count}"
            )
            if fail_count > 0:
                detail_msg = "\n".join(failed_files[:5])
                ...
                self.root.after(
                    ...
                    lambda: [
                        ...
                        messagebox.showerror("上传结果", full_msg),
                    ],
                )
            else:
                self.root.after(
