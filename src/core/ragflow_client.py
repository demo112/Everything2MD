    ...
    def _handle_response(self, response, url=None):
        try:
            ...
            if data.get("code", -1) != 0:
                raise Exception(
                    f"API Error (URL: {url}): {data.get('message', 'Unknown error')}"
                )
            ...
            if (
                str(e).startswith("API Error")
                or str(e).startswith("HTTP Error")
                or str(e).startswith("Invalid JSON")
            ):
                raise
        ...
        try:
            with httpx.Client(timeout=10.0) as client:
                ...
                if template_id:
                    ...
                    log_info(
                        f"Template application not yet fully implemented. Created empty dataset: {name}"
                    )
