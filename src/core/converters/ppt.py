...
class PptConverter(BaseConverter):
    ...
    def convert(self, input_path: Path, output_path: Path, context=None, **kwargs) -> Path:
        ...
        if output_path.suffix.lower() == '.pdf':
            if suffix == '.pdf':
                ...
                return self._convert_ppt(input_path, output_path, context, output_pdf_only=True)
    ...
    def _convert_pptx(self, input_path, output_path, context=None):
        ...
        try:
            ...
            try:
                ...
                outputter = md_outputter(str(output_path), image_dir=str(img_dir), image_page_dir_check=True)
    ...
    def _convert_ppt(self, input_path, output_path, context=None, output_pdf_only=False):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                result = self._run_subprocess(cmd, context=context, check=False, capture_output=True, timeout=120)
                if result.returncode != 0:
                     raise RuntimeError(f"LibreOffice转换PDF失败 (Exit Code {result.returncode}): {result.stderr.decode() or result.stdout.decode()}")
            ...
            if not pdf_files:
                # 记录更多调试信息
                log_warn(f"LibreOffice执行结束但未生成PDF。Stdout: {result.stdout.decode()} Stderr: {result.stderr.decode()}")
    ...
    def _convert_pdf_to_md(self, pdf_file: Path, output_path: Path, context=None) -> Path:
        # 优先使用 pandoc
        ...
        if pandoc_path:
            ...
            try:
                self._run_subprocess(cmd_pandoc, context=context, check=True, capture_output=True)
                pandoc_success = True
        ...
        if not pandoc_success:
            ...
            if shutil.which("pdftotext"):
                try:
                    self._run_subprocess(["pdftotext", "-layout", "-enc", "UTF-8", str(pdf_file), str(output_path)], context=context, check=True)
                except subprocess.CalledProcessError as e:
