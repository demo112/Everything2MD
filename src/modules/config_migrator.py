...
def migrate_ini_to_json(ini_path, json_path):
    ...
    default_config = {
        ...
        "path_settings": {"last_input_path": "", "last_output_path": ""},
    }
    ...
    if ini_path.exists():
        ...
        if config_parser.has_section("general"):
            if config_parser.has_option("general", "log_level"):
                default_config["conversion_settings"]["log_level"] = config_parser.get(
                    "general", "log_level"
                )

            if config_parser.has_option("general", "output_format"):
                default_config["conversion_settings"]["output_format"] = (
                    config_parser.get("general", "output_format")
                )

        # 迁移批量处理配置
        if config_parser.has_section("batch"):
            if config_parser.has_option("batch", "enabled"):
                default_config["conversion_settings"]["batch_processing"]["enabled"] = (
                    config_parser.getboolean("batch", "enabled")
                )
