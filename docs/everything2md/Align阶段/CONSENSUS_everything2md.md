...
2. **文件类型检测**：根据文件扩展名确定处理路径，支持扩展名和MIME类型双重检测
3. **LibreOffice转换器**：处理 .doc, .docx, .ppt 文件，包含中文文件名安全处理
4. **ppt转换器**：处理 .ppt 文件
5. **pptx2md转换器**：处理 .pptx 文件
6. **Pandoc转换器**：将中间格式转换为 Markdown
7. **直接复制模块**：处理 .txt 和 .md 文件
8. **依赖检查系统**：验证运行环境
9. **批量处理模块**：实现批量文件处理功能
10. **配置管理系统**：处理用户配置选项
11. **错误处理模块**：统一的错误处理和日志记录
12. **日志记录模块**：提供分级日志功能
