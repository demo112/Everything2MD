...
class FixedEverything2MDGUI:
    ...
    def __init__(self, root):
        ...
        self.config_manager_path = os.path.join(
            self.project_root, "src", "modules", "config_manager.sh"
        )
    ...
    def create_widgets(self):
        ...
        # 基本设置标签页内容
        ttk.Label(basic_frame, text="日志级别:").grid(
        ...
        ttk.Label(basic_frame, text="输出格式:").grid(
        ...
        # 批量处理标签页内容
        ttk.Checkbutton(
            batch_frame, text="启用批量处理", variable=self.batch_processing
        ).grid(row=0, column=0, columnspan=2, sticky=tk.W, padx=5, pady=5)

        ttk.Label(batch_frame, text="最大并行任务数:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=5
        )
        max_parallel_jobs_spin = ttk.Spinbox(
            batch_frame, from_=1, to=16, textvariable=self.max_parallel_jobs, width=10
        )
        max_parallel_jobs_spin.grid(row=1, column=1, sticky=tk.W, padx=5, pady=5)

        ttk.Label(batch_frame, text="文件过滤器:").grid(
            row=2, column=0, sticky=tk.W, padx=5, pady=5
        )
        file_filters_entry = ttk.Entry(
            batch_frame, textvariable=self.file_filters, width=30
        )
        file_filters_entry.grid(row=2, column=1, sticky=tk.W, padx=5, pady=5)
        ttk.Label(batch_frame, text="格式: docx,pptx,pdf,txt").grid(
            row=3, column=1, sticky=tk.W, padx=5, pady=0
        )

        # 路径设置标签页内容
        ttk.Label(path_frame, text="输入路径:").grid(
            row=0, column=0, sticky=tk.W, padx=5, pady=5
        )
        input_path_frame = ttk.Frame(path_frame)
        input_path_frame.grid(row=0, column=1, sticky=tk.W, padx=5, pady=5)
        ttk.Entry(input_path_frame, textvariable=self.input_path, width=30).pack(
            side=tk.LEFT
        )
        ttk.Button(input_path_frame, text="浏览", command=self.browse_input_path).pack(
            side=tk.LEFT, padx=(5, 0)
        )

        ttk.Label(path_frame, text="输出路径:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=5
        )
        output_path_frame = ttk.Frame(path_frame)
        output_path_frame.grid(row=1, column=1, sticky=tk.W, padx=5, pady=5)
        ttk.Entry(output_path_frame, textvariable=self.output_path, width=30).pack(
            side=tk.LEFT
        )
        ttk.Button(
            output_path_frame, text="浏览", command=self.browse_output_path
        ...
        ttk.Button(
            button_frame, text="恢复默认设置", command=self.restore_defaults
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="保存配置", command=self.save_config).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(button_frame, text="开始转换", command=self.start_conversion).pack(
            side=tk.RIGHT
        )
    ...
    def load_config(self):
        ...
        try:
            ...
            self.batch_processing.set(
                self.get_config_value(cmd, "batch_processing_enabled") == "true"
            )
    ...
    def get_config_value(self, cmd, key):
        ...
        try:
            result = subprocess.run(
                cmd + ["get_config", key], capture_output=True, text=True, check=True
            )
        ...
        try:
            ...
            config_values = {
                ...
                "batch_processing_enabled": (
                    "true" if self.batch_processing.get() else "false"
                ),
            ...
            for key, value in config_values.items():
                ...
                try:
                    ...
                    result = subprocess.run(
                        validation_cmd, capture_output=True, text=True
                    )
                    if result.returncode != 0:
                        error_msg = (
                            result.stderr.strip() if result.stderr else "验证失败"
                        )
    ...
    def start_conversion(self):
        ...
        try:
            conversion_cmd = [
                "bash",
                os.path.join(
                    self.project_root, "src", "modules", "conversion_manager.sh"
                ),
                input_path,
                output_path,
            ]
            ...
            thread = threading.Thread(
                target=self.run_conversion, args=(conversion_cmd,)
            )
            thread.daemon = True
            thread.start()
