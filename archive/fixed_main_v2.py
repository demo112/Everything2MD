...
class FixedEverything2MDGUI:
    ...
    def load_config(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                self.config["output_format"].set(
                    config_data.get("output_format", "markdown")
                )

                batch_settings = config_data.get("conversion_settings", {}).get(
                    "batch_processing", {}
                )
                self.config["batch_processing_enabled"].set(
                    batch_settings.get("enabled", True)
                )
                self.config["max_parallel_jobs"].set(
                    str(batch_settings.get("max_parallel_jobs", 2))
                )
                self.config["file_filters"].set(
                    ",".join(
                        batch_settings.get(
                ...
                self.config["last_input_path"].set(
                    config_data.get("last_input_path", "")
                )
                self.config["last_output_path"].set(
                    config_data.get("last_output_path", "")
                )
                ...
                "conversion_settings": {
                    "batch_processing": {
                        ...
                        "max_parallel_jobs": int(
                            self.config["max_parallel_jobs"].get()
                        ),
                        "file_filters": [
                            f.strip()
                            for f in self.config["file_filters"].get().split(",")
                            if f.strip()
                        ],
                    }
                },
    ...
    def create_general_tab(self, parent):
        ...
        # Log level
        ttk.Label(parent, text="日志级别:").grid(
            row=0, column=0, sticky=tk.W, padx=5, pady=5
        )
        log_level_combo = ttk.Combobox(
            parent,
            textvariable=self.config["log_level"],
        ...
        output_format_combo = ttk.Combobox(
            parent,
            textvariable=self.config["output_format"],
    ...
    def create_batch_tab(self, parent):
        ...
        # Max parallel jobs
        ttk.Label(parent, text="最大并行任务数:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=5
        )
        max_jobs_spin = ttk.Spinbox(
            ...
            from_=1,
            to=16,
            textvariable=self.config["max_parallel_jobs"],
            width=10,
        )
        max_jobs_spin.grid(row=1, column=1, sticky=tk.W, padx=5, pady=5)

        # File filters
        ttk.Label(parent, text="文件过滤器:").grid(
            row=2, column=0, sticky=tk.W, padx=5, pady=5
        )
        filters_entry = ttk.Entry(
            parent, textvariable=self.config["file_filters"], width=40
        )
    ...
    def create_paths_tab(self, parent):
        """Create paths tab"""
        # Input path
        ttk.Label(parent, text="输入路径:").grid(
            row=0, column=0, sticky=tk.W, padx=5, pady=5
        )
        ...
        # Output path
        ttk.Label(parent, text="输出路径:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=5
        )
